...
static Value *getFieldAddress(IRBuilder<> &Builder, Value *Base,
  ...
  // The base value should be an i8*.
...
CallSite ABICallSignature::emitUnmanagedCall(GenIR &Reader, Value *Target,
  ...
  if (Reader.UnmanagedCallFrame == nullptr) {
    Reader.insertIRForUnmanagedCallFrame();
    assert(Reader.UnmanagedCallFrame != nullptr);
  }
  ...
  if (Reader.MethodSignature.hasSecretParameter()) {
    // TODO: this needs to be updated for direct unmanaged calls.
  ...
  SmallVector<Value *, 16> IntrinsicArgs(3 + TargetArgCount + 6);
  ...
  IntrinsicArgs[J] = ConstantInt::get(Int32Ty, 4);
...
Value *ABICallSignature::emitCall(GenIR &Reader, Value *Target, bool MayThrow,
                                  ArrayRef<Value *> Args,
                                  Value *IndirectionCell, Value *PInvokeCookie,
  ...
  bool HasPInvokeCookie = PInvokeCookie != nullptr;
  bool IsUnmanagedCall =
      Signature.getCallingConvention() != CORINFO_CALLCONV_DEFAULT;
  assert(((HasIndirectionCell ? 1 : 0) + (HasPInvokeCookie ? 1 : 0) +
          (IsUnmanagedCall ? 1 : 0)) <= 1);

  uint32_t NumSpecialArgs = 0;
  if (HasIndirectionCell) {
    NumSpecialArgs = 1;
  } else if (HasPInvokeCookie) {
    // PInvoke calls made via the helper have 2 extra parameters:
    // - The PInvoke cookie
    // - The call target
    NumSpecialArgs = 2;
  }
  ...
  // arguments. The backend will place these arguments in the appropriates
  ...
  if (HasIndirectionCell) {
    ...
    assert(
        PInvokeCookie->getType()->isIntegerTy(Reader.TargetPointerSizeInBits));

    // TODO: we may need to place the PInvoke cookie at the end for X86 in order
    //       for the backend to put it in the right stack slot.
    ArgumentTypes[0] = PInvokeCookie->getType();
    Arguments[0] = PInvokeCookie;

    ArgumentTypes[1] = Target->getType();
    Arguments[1] = Target;

    Target = (Value *)Reader.getHelperCallAddress(CORINFO_HELP_PINVOKE_CALLI);
  }

  int32_t ResultIndex = -1;
  if (HasIndirectResult) {
    ResultIndex = (int32_t)NumSpecialArgs;
  ...
  // Unmanaged calls require an intrinsic in order to ensure that any
  // callee-save registers that might contain GC pointers are spilled.
  CallSite Call;
  ...
  if (HasIndirectionCell) {
    ...
    assert(getLLVMCallingConv(Signature.getCallingConvention()) ==
           CallingConv::C);
    CC = CallingConv::C;
    // CC = CallingConv::CLR_PInvoke;
  } else {
